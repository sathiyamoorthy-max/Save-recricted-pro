    ...
    if state:
        ...
        if step == "PAYMENT_UTR":
            ...
            if payment:
                ...
                alert = (
                    f"💳 **NEW PAYMENT — MANUAL VERIFICATION REQUIRED**\n\nInvoice: `{invoice_id}`\nUser: `{user_id}`\n"
                    f"Plan: **{payment['plan']}**\nDuration: **{payment['duration']}**\n"
                    f"Amount: **₹{payment['amount']}**\nUTR: `{payment['utr']}`\n\n"
                    "⚠️ Check your actual bank/UPI receipt before approving. A UTR/screenshot can be forged."
                )
                for a in {int(r["user_id"]) for r in admins}:
                    try:
                        if payment.get("screenshot_file_id"):
                            await bot.send_photo(a, payment["screenshot_file_id"], caption=alert, reply_markup=markup)
                        else:
