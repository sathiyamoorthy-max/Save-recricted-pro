...
def main() -> None:
    ...
    try:
        # IMPORTANT: Kurigram/Pyrogram binds its Client to the event loop that
        # exists when the Client is created. Using asyncio.run() creates a new
        # loop and can cause "Future attached to a different loop" on Python 3.12.
        # Client.run(coroutine) executes the coroutine on the Client's own loop.
