...
APP_NAME = os.getenv("APP_NAME", "SavePro V4 Hybrid Advanced").strip()
BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip()
API_ID_RAW = os.getenv("API_ID", "").strip()
API_HASH = os.getenv("API_HASH", "").strip()
API_ID = int(API_ID_RAW) if API_ID_RAW.isdigit() else 0
BOT_USERNAME = os.getenv("BOT_USERNAME", "").lstrip("@").strip()
ADMIN_USERNAME_ENV = os.getenv("ADMIN_USERNAME", "toonworldvsm2").lstrip("@").strip()
...
SUPPORT_USERNAME_ENV = os.getenv("SUPPORT_USERNAME", ADMIN_USERNAME_ENV or "admin").lstrip("@").strip()
...
PLAN_PRICING: dict[str, dict[str, Any]] = {
    ...
    "Ultimate": {
        ...
        "Monthly": 500,
        "Yearly": 4200,
        "access": "Unlimited + highest priority + Full Clone",
    },
}
...
MANUAL = (
    "📖 **SavePro Advanced User Guide**\n\n"
    "1️⃣ `/login` — 2 methods: Session String or Phone + OTP (+ 2FA when enabled).\n"
    "2️⃣ **Single:** send an accessible Telegram post link directly.\n"
    "3️⃣ **Batch:** `/batch <first_link> <last_link>` or use the interactive wizard.\n"
    "4️⃣ **Full Clone:** `/clone <start_post_link>` — start → latest accessible message.\n"
    "5️⃣ **Channel Clone:** `/clone <source> <destination> [limit|all]`.\n"
    "6️⃣ `/jobs` / `/pause JOB...` / `/resume JOB...` / `/cancel JOB...` — job control.\n"
    "7️⃣ `/settings` — language, media filter, thumbnails, caption and alerts.\n"
    "8️⃣ `/plans` / `/myplan` / `/paymenthistory` — subscription & payments.\n"
    "9️⃣ `/referral` / `/bonus` — rewards. `/support` — support ticket.\n\n"
    "🔐 Use only content/chats your connected account is legitimately allowed to access. "
    "Source admin status is not required; destination posting must be allowed by Telegram. "
    "Never share OTP, 2FA password or session string with anyone else."
)
...
def extract_telegram_links(text: str) -> list[str]:
    """Extract Telegram post links, including accidentally glued links."""
    fixed = re.sub(r"(\d)(https?://t\.me/)", r"\1 \2", text or "")
    links = re.findall(r"https?://t\.me/[^\s<>]+", fixed)
    return [x.rstrip(").,]}>") for x in links]


def parse_tg_link_ex(text: str) -> Optional[tuple[str, Optional[int], int]]:
    """Return (chat_ref, topic_id, message_id) for public/private Telegram post links."""
    links = extract_telegram_links(text)
    if not links:
        return None
    url = links[0].split("?", 1)[0].strip().rstrip("/")
    try:
    ...
    # Private / internal links: t.me/c/<internal_id>/<msg_id>
    # Forum topic links:       t.me/c/<internal_id>/<topic_id>/<msg_id>
    if parts[0] == "c":
    ...
    if len(nums) >= 2 and nums[-2].isdigit():
        topic_id = int(nums[-2])
    return chat_ref, topic_id, msg_id


def parse_tg_link(text: str) -> Optional[tuple[str, int]]:
    """Backward-compatible two-value parser used by legacy flows."""
    parsed = parse_tg_link_ex(text)
    if not parsed:
        return None
...
def chat_ref_to_id(chat_raw: str) -> Any:
    """Normalize username, numeric IDs, t.me chat links and private c-links."""
    raw = str(chat_raw).strip().strip("<>")
    if "t.me/" in raw:
        raw = raw.split("t.me/", 1)[1].split("?", 1)[0].strip("/")
        parts = [p for p in raw.split("/") if p]
        if parts:
            if parts[0] == "c" and len(parts) >= 2 and parts[1].isdigit():
                return int(f"-100{parts[1]}")
            if parts[0] not in {"joinchat"} and not parts[0].startswith("+"):
                raw = parts[0]
    if raw.startswith("@"):
        raw = raw[1:]
    if raw.isdigit():
...
async def init_db() -> None:
    async with DB_LOCK:
        async with db_conn() as db:
            ...
            for _plan, _values in PLAN_PRICING.items():
                for _duration in DURATION_DAYS:
                    defaults[f"price:{_plan}:{_duration}"] = str(_values[_duration])
...
def price_setting_key(plan: str, duration: str) -> str:
    return f"price:{plan}:{duration}"


async def get_plan_price(plan: str, duration: str) -> int:
    if plan not in PLAN_PRICING or duration not in DURATION_DAYS:
        raise ValueError("Invalid plan or duration")
    default = int(PLAN_PRICING[plan][duration])
    raw = await get_setting(price_setting_key(plan, duration), str(default))
    try:
...
async def resolve_configured_admin_username() -> None:
    """Resolve ADMIN_USERNAME to a numeric Telegram user id at startup."""
    if not ADMIN_USERNAME_ENV:
        return
    try:
        admin_user = await bot.get_users(ADMIN_USERNAME_ENV)
        if getattr(admin_user, "is_bot", False):
            log.warning(
                "ADMIN_USERNAME @%s belongs to a bot, not a human admin; "
                "set OWNER_ID/ADMIN_ID to your numeric personal Telegram user id.",
                ADMIN_USERNAME_ENV,
            )
            return
        aid = int(admin_user.id)
        EXTRA_ADMIN_IDS.add(aid)
        role = "owner" if not OWNER_ID else "manager"
        async with DB_LOCK:
            async with db_conn() as db:
                await db.execute(
                    "INSERT INTO admins(user_id, role, added_by, created_at) VALUES (?, ?, ?, ?) "
                    "ON CONFLICT(user_id) DO UPDATE SET role=excluded.role",
                    (aid, role, OWNER_ID or aid, now_iso()),
                )
                await db.commit()
        log.info("Configured admin @%s resolved to user id %s", ADMIN_USERNAME_ENV, aid)
    except Exception as exc:
        log.warning("Could not resolve ADMIN_USERNAME @%s: %s", ADMIN_USERNAME_ENV, exc)


async def payment_admin_recipient_ids() -> set[int]:
    """Return every reachable human admin configured by numeric ID or ADMIN_USERNAME."""
    recipients: set[int] = set(EXTRA_ADMIN_IDS)
    if OWNER_ID:
        recipients.add(int(OWNER_ID))
    try:
        async with db_conn() as db:
            rows = await (await db.execute(
                "SELECT user_id FROM admins WHERE role IN ('owner','manager','payments')"
            )).fetchall()
        recipients.update(int(r["user_id"]) for r in rows)
    except Exception:
        pass
    ...
    if ADMIN_USERNAME_ENV:
        try:
            admin_user = await bot.get_users(ADMIN_USERNAME_ENV)
            if not getattr(admin_user, "is_bot", False):
                recipients.add(int(admin_user.id))
        except Exception as exc:
            log.warning("Could not resolve payment admin @%s: %s", ADMIN_USERNAME_ENV, exc)
    return {uid for uid in recipients if uid > 0}
...
async def expire_user_if_needed(user_id: int) -> None:
    if is_admin_id(user_id):
        return
    expired_plan: Optional[str] = None
    ...
    async with DB_LOCK:
        async with db_conn() as db:
            ...
            if row["plan"] != "Free" and expiry and now_dt() >= expiry:
                expired_plan = str(row["plan"])
                expired_at = str(row["plan_expires_at"])
                await db.execute(
    ...
    if not expired_plan or not expired_at:
        return
    ...
    async with DB_LOCK:
        async with db_conn() as db:
            cur = await db.execute(
                "SELECT 1 FROM subscription_reminders WHERE user_id=? AND expiry=? AND reminder_type='expired'",
                (user_id, expired_at),
            )
            if await cur.fetchone():
                ...
                await db.execute(
                    "INSERT INTO subscription_reminders(user_id,expiry,reminder_type,sent_at) VALUES (?,?, 'expired', ?)",
                    (user_id, expired_at, now_iso()),
                )
                await db.commit()

    if should_notify and getattr(bot, "is_connected", False):
        try:
            if expired_plan == "Trial":
                text = (
                    "⏰ **FREE TRIAL FINISHED**\n\n"
                    "Your 24-hour trial has ended. Buy a subscription to continue paid features."
                )
            else:
                text = (
                    f"⏰ **SUBSCRIPTION EXPIRED**\n\nYour **{expired_plan}** plan has ended. "
                    "Renew to continue paid features."
                )
            await bot.send_message(
                user_id,
                text,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("💎 Buy Subscription", callback_data="home_plans")
                ]]),
            )
        except Exception:
            pass
...
async def send_upgrade_required(msg: Message, reason: str = "limit") -> None:
    """Show a clear upgrade prompt before a job is queued when usage is unavailable."""
    if reason == "trial":
        body = "⏰ **Your free trial has finished.**"
    else:
        body = "⛔ **Your trial or plan limit has finished.**"
    await msg.reply_text(
        body + "\n\nPlease upgrade or renew your plan to continue.",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("💎 Upgrade / Renew", callback_data="home_plans")
        ]]),
    )


async def usage_available_or_notify(user_id: int, msg: Message) -> bool:
    """Preflight usage gate so zero-credit jobs never get queued as 0-success jobs."""
    await expire_user_if_needed(user_id)
    user = await get_user(user_id)
    if is_admin_id(user_id) or user.get("plan") == "Ultimate":
        return True
    # Once the one-time trial has expired and the account has fallen back to Free,
    # do not silently start free jobs. Show the upgrade prompt immediately.
    if str(user.get("plan") or "") == "Free" and int(user.get("trial_used") or 0):
        await send_upgrade_required(msg, "trial")
        return False
    if await can_consume(user_id):
        return True
    await send_upgrade_required(msg, "limit")
    return False
...
async def create_invoice(user_id: int, plan: str, duration: str) -> dict[str, Any]:
    ...
    amount = await get_plan_price(plan, duration)
    async with DB_LOCK:
        async with db_conn() as db:
            await db.execute(
                """
...
async def submit_utr(user_id: int, invoice_id: str, utr: str) -> tuple[bool, str]:
    utr = utr.strip().upper()
    # UPI bank RRN/UTR used by this bot must be exactly 12 numeric digits.
    # Format validation alone cannot prove that money was received; admin verification is mandatory.
    if not re.fullmatch(r"[0-9]{12}", utr):
        return False, "UPI UTR / RRN must be exactly 12 digits."
    async with DB_LOCK:
        async with db_conn() as db:
    ...
    async with DB_LOCK:
        async with db_conn() as db:
            ...
            if not payment.get("screenshot_file_id"):
                return False, "Payment screenshot is required before approval", payment
            await db.execute(
...
async def latest_active_job_id(user_id: int) -> Optional[str]:
    async with db_conn() as db:
        cur = await db.execute(
            "SELECT job_id FROM jobs WHERE user_id=? AND status IN ('running','queued','paused') "
            "ORDER BY CASE status WHEN 'running' THEN 0 WHEN 'queued' THEN 1 ELSE 2 END, id DESC LIMIT 1",
            (user_id,),
        )
        row = await cur.fetchone()
...
async def resume_job(job_id: str, user_id: int) -> bool:
    ...
    if job["status"] != "paused":
        return False
    # If it had already started, the live worker is waiting on the status.
...
async def wait_for_job_control(job_id: str) -> bool:
    """Return False when cancelled; block while paused."""
    while True:
...
async def recover_jobs() -> None:
    ...
    async with DB_LOCK:
        async with db_conn() as db:
            # Running jobs are safely re-queued after a restart.
            await db.execute("UPDATE jobs SET status='queued', started_at=NULL WHERE status='running'")
            # Paused jobs stay paused, but started_at is cleared so /resume can re-queue them.
            await db.execute("UPDATE jobs SET started_at=NULL WHERE status='paused'")
...
async def resolve_target_peer(client: Client, chat_id: Any) -> Any:
    ...
    try:
        async for dialog in client.get_dialogs(limit=400):
            username = str(dialog.chat.username or "").lower().lstrip("@")
            wanted = str(chat_id).lower().lstrip("@")
            if str(dialog.chat.id) == str(chat_id) or (username and username == wanted):
...
async def warm_peer_cache(client: Client, limit: int = 400) -> None:
    """Refresh accessible dialogs before retrying a peer lookup."""
    try:
...
async def fetch_message_safely(client: Client, chat_id: Any, msg_id: int) -> Message:
    """Use the connected user's legitimate access and retry once after peer-cache refresh."""
    try:
        return await client.get_messages(chat_id, msg_id)
    except Exception:
        await warm_peer_cache(client)
        peer = await resolve_target_peer(client, chat_id)
        return await client.get_messages(peer, msg_id)
...
async def progress_tracker(
    current: int, total: int, status_msg: Message, label: str, last: list[float],
    job_id: Optional[str] = None, tx_client: Optional[Client] = None,
) -> None:
    if job_id and job_id in CANCEL_REQUESTS:
        if tx_client is not None:
            try:
                tx_client.stop_transmission()
            except Exception:
                pass
        return
    now = time.time()
    ...
    try:
        await status_msg.edit_text(
            ...
            + (f"Cancel: `/cancel {job_id}`" if job_id else "")
        )
    except Exception:
        pass
...
async def deliver_message_media(
    ...
    if not target or getattr(target, "empty", False) or target.service:
        return False
    ...
    f_path = await target.download(
        ...
        progress_args=(status_msg, f"Downloading {item_label}", last_d, job_id, user_client),
    )
    ...
    try:
        if target.document:
            await bot.send_document(
                user_id, f_path, caption=caption, thumb=thumb_doc,
                progress=progress_tracker, progress_args=(status_msg, f"Uploading {item_label}", last_u, job_id, bot),
            )
        elif target.video:
            await bot.send_video(
                user_id, f_path, caption=caption, thumb=thumb_vid,
                duration=getattr(target.video, "duration", None),
                width=getattr(target.video, "width", None),
                height=getattr(target.video, "height", None),
            ...
            await bot.send_audio(
                user_id, f_path, caption=caption, thumb=thumb_doc,
                duration=getattr(target.audio, "duration", None),
                performer=getattr(target.audio, "performer", None),
                title=getattr(target.audio, "title", None),
                progress=progress_tracker, progress_args=(status_msg, f"Uploading {item_label}", last_u, job_id, bot),
            )
        elif target.voice:
            await bot.send_voice(
                user_id, f_path, caption=caption,
                duration=getattr(target.voice, "duration", None),
            )
        elif target.photo:
            await bot.send_photo(user_id, f_path, caption=caption)
        else:
            await bot.send_document(user_id, f_path, caption=caption, thumb=thumb_doc)
        return True
    finally:
        try:
            os.remove(f_path)
...
async def execute_save_job(job: dict[str, Any]) -> None:
    """Single/Batch/Full-Clone engine based on the proven connected-user download method."""
    ...
    end_id = int(job["end_id"] or start_id)
    job_id = str(job["job_id"])
    job_type = str(job.get("job_type") or "single")
    ...
    status_msg = await bot.send_message(user_id, f"⏳ **{job_id} starting…**")
    ...
    try:
        ...
        # True Full Clone: start-link -> latest accessible message.
        if job_type == "clone_full":
            latest_id = start_id
            async for last_m in u_client.get_chat_history(peer, limit=1):
                latest_id = int(last_m.id)
            end_id = max(start_id, latest_id)

        total = max(1, end_id - start_id + 1)
        await update_job(job_id, progress_total=total)
        mode = "Full Clone" if job_type == "clone_full" else ("Batch" if total > 1 else "Single")
        topic_text = f"\nTopic: `{topic_id}`" if topic_id else ""
        await status_msg.edit_text(
            f"🚀 **{mode} started**\n"
            f"Range: `#{start_id}` → `#{end_id}`\nItems scanned: **{total}**{topic_text}"
        )

        for idx, cur_id in enumerate(range(start_id, end_id + 1), start=1):
            if not await wait_for_job_control(job_id):
                await status_msg.edit_text(
            ...
            try:
                target = await fetch_message_safely(u_client, peer, cur_id)
                if not target or getattr(target, "empty", False):
                    skipped += 1
                elif topic_id and message_topic_id(target) not in {topic_id, None if target.id == topic_id else -1} and target.id != topic_id:
                    skipped += 1
                else:
                    sent = await deliver_message_media(
                        u_client, target, user, status_msg, job_dir, f"#{cur_id}", job_id
                    )
                    if sent:
                ...
                try:
                    target = await fetch_message_safely(u_client, peer, cur_id)
                    if target and not getattr(target, "empty", False):
                        if topic_id and message_topic_id(target) not in {topic_id, None if target.id == topic_id else -1} and target.id != topic_id:
                            skipped += 1
                        else:
                            sent = await deliver_message_media(
                                u_client, target, user, status_msg, job_dir, f"#{cur_id}", job_id
                    ...
                    failed += 1
                    await log_error(user_id, f"job_retry:{job_id}:{cur_id}", exc)
            ...
            if idx % 5 == 0 or idx == total:
                try:
                    await status_msg.edit_text(
                        ...
                        f"Use `/pause {job_id}`, `/resume {job_id}` or `/cancel {job_id}`."
                    )
                except Exception:
                    pass
        ...
        if latest and latest["status"] != "cancelled":
            if job_type == "clone_full":
                title = "✅ **Full Clone முடிந்தது!**"
            elif total > 1:
                title = "✅ **Batch முடிந்தது!**"
            else:
                title = "✅ **Single Save முடிந்தது!**"
            await status_msg.edit_text(
                f"{title}\n\n"
                f"Job: `{job_id}`\n"
                f"Range: `#{start_id}` → `#{end_id}`\n"
...
async def execute_transfer_job(job: dict[str, Any]) -> None:
    """Clone accessible source history. Source needs read access; destination needs Telegram-granted posting permission."""
    user_id = int(job["user_id"])
    user = await get_user(user_id)
    if user.get("plan") != "Ultimate" and not is_admin_id(user_id):
        raise RuntimeError("Full channel clone requires Ultimate plan")

    src = chat_ref_to_id(str(job["source"]))
    dest = chat_ref_to_id(str(job["destination"]))
    job_id = str(job["job_id"])
    status_msg = await bot.send_message(user_id, f"🔄 **{job_id}** validating source/destination access…")
    u_client: Optional[Client] = None
    job_dir = TEMP_DIR / job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    success = failed = 0
    try:
        ...
        try:
            ...
            raise PermissionError("Source is not accessible to the connected Telegram account.") from exc
        try:
            await u_client.get_chat(dest)
        except Exception as exc:
            raise PermissionError("Destination is not accessible to the connected Telegram account.") from exc
        ...
        try:
            ...
            raw_limit = 500
        history_limit = 0 if raw_limit <= 0 else min(raw_limit, 20000)

        await status_msg.edit_text(
            f"🚀 **{job_id} channel clone started**\n"
            f"Limit: **{'ALL' if history_limit == 0 else history_limit}**\n"
            "Source admin is not required; Telegram destination posting permission is required."
        )

        async for post in u_client.get_chat_history(src, limit=history_limit):
            if not await wait_for_job_control(job_id):
                break
            try:
                ...
                if post.media:
                    ...
                    try:
                        if post.document:
                            ...
                            await u_client.send_video(
                                dest, f, caption=caption, thumb=get_valid_thumb(user.get("vid_thumb")),
                                duration=getattr(post.video, "duration", None),
                                width=getattr(post.video, "width", None),
                                height=getattr(post.video, "height", None),
                                supports_streaming=True,
                            )
                        elif post.audio:
                            await u_client.send_audio(
                                dest, f, caption=caption, thumb=get_valid_thumb(user.get("doc_thumb")),
                                duration=getattr(post.audio, "duration", None),
                                performer=getattr(post.audio, "performer", None),
                                title=getattr(post.audio, "title", None),
                            )
                        elif post.voice:
                            await u_client.send_voice(dest, f, caption=caption, duration=getattr(post.voice, "duration", None))
                        elif post.photo:
                            await u_client.send_photo(dest, f, caption=caption)
                        else:
                            await u_client.send_document(dest, f, caption=caption)
                        success += 1
                    finally:
                        try:
                            os.remove(f)
        ...
        latest = await get_job(job_id)
        if latest and latest["status"] == "cancelled":
            await status_msg.edit_text(f"🚫 **Channel clone cancelled**\n✅ {success} | ❌ {failed}")
        else:
            await status_msg.edit_text(
                f"✅ **Channel Clone முடிந்தது!**\n\nJob: `{job_id}`\n"
...
async def job_worker(worker_no: int) -> None:
    ...
    while not SHUTDOWN_EVENT.is_set():
        ...
        try:
            ...
            try:
                if job["job_type"] in {"single", "batch", "clone_full"}:
                    timeout = max(JOB_TIMEOUT_SECONDS, 21600) if job["job_type"] == "clone_full" else JOB_TIMEOUT_SECONDS
                    await asyncio.wait_for(execute_save_job(job), timeout=timeout)
                elif job["job_type"] == "transfer":
                    await asyncio.wait_for(execute_transfer_job(job), timeout=max(JOB_TIMEOUT_SECONDS, 21600))
                else:
...
async def render_plans(message: Message, edit: bool = False) -> None:
    ...
    for plan in PLAN_PRICING:
        prices[plan] = {d: await get_plan_price(plan, d) for d in DURATION_DAYS}
    text = (
        "💎 **PLANS & PRICING**\n\n"
        f"⚡ **Basic** — 10 jobs/day\nWeekly ₹{prices['Basic']['Weekly']} | Monthly ₹{prices['Basic']['Monthly']} | Yearly ₹{prices['Basic']['Yearly']}\n\n"
        f"🥈 **Standard** — 50 jobs/day + batch\nWeekly ₹{prices['Standard']['Weekly']} | Monthly ₹{prices['Standard']['Monthly']} | Yearly ₹{prices['Standard']['Yearly']}\n\n"
        f"🥇 **Premium** — 100 jobs/day + priority batch\nWeekly ₹{prices['Premium']['Weekly']} | Monthly ₹{prices['Premium']['Monthly']} | Yearly ₹{prices['Premium']['Yearly']}\n\n"
        f"🔷 **Ultimate** — Unlimited + Full Clone + highest priority\nWeekly ₹{prices['Ultimate']['Weekly']} | Monthly ₹{prices['Ultimate']['Monthly']} | Yearly ₹{prices['Ultimate']['Yearly']}\n\n"
...
async def cancel_handler(_: Client, msg: Message) -> None:
    ...
    if len(args) > 1:
        ...
        if raw == "ALL":
            ...
            cancelled = 0
            for row in rows:
                if await cancel_job(str(row["job_id"]), user_id):
                    cancelled += 1
            await msg.reply_text(f"🚫 Cancelled **{cancelled}** active job(s)." if cancelled else "ℹ️ No active jobs to cancel.")
            return
        if raw.startswith("JOB-"):
            target_job = raw
    else:
        target_job = await latest_active_job_id(user_id)

    if target_job:
        ok = await cancel_job(target_job, user_id)
        if ok:
            await msg.reply_text(
                f"🚫 **Cancellation requested:** `{target_job}`\n"
                "The current file transfer will stop as soon as Telegram releases the active transmission."
            )
            return

    if state:
        await msg.reply_text("✅ Current input/login flow cancelled.")
    else:
        await msg.reply_text("ℹ️ No active input flow or job to cancel.")
...
async def pause_handler(_: Client, msg: Message) -> None:
    ...
    args = (msg.text or "").split(maxsplit=1)
    if len(args) < 2:
        await msg.reply_text("Usage: `/pause JOB-...`")
        return
    ok = await pause_job(args[1].strip().upper(), msg.from_user.id)
    await msg.reply_text("⏸ Job paused." if ok else "❌ Job not found or cannot be paused.")
...
async def resume_handler(_: Client, msg: Message) -> None:
    ...
    args = (msg.text or "").split(maxsplit=1)
    if len(args) < 2:
        await msg.reply_text("Usage: `/resume JOB-...`")
        return
    ok = await resume_job(args[1].strip().upper(), msg.from_user.id)
    await msg.reply_text("▶️ Job resumed." if ok else "❌ Job not found or cannot be resumed.")


@bot.on_message(filters.command("id") & filters.private)
async def id_handler(_: Client, msg: Message) -> None:
    if msg.from_user:
        await msg.reply_text(f"🆔 Your Telegram User ID: `{msg.from_user.id}`")


@bot.on_message(filters.command("referral") & filters.private)
async def referral_handler(_: Client, msg: Message) -> None:
    if not msg.from_user:
        return
    user = await get_user(msg.from_user.id)
    bot_name = BOT_USERNAME or (await bot.get_me()).username or "YourBot"
    link = f"https://t.me/{bot_name}?start=ref_{msg.from_user.id}"
    await msg.reply_text(
        "🎁 **REFERRALS**\n\n"
        f"Referrals: **{user.get('ref_count', 0)}**\n"
        f"Bonus credits: **{user.get('bonus_credits', 0)}**\n\n"
        f"Your link:\n`{link}`\n\n"
        "Each valid new referral adds +2 bonus credits."
    )


@bot.on_message(filters.command("bonus") & filters.private)
async def bonus_handler(_: Client, msg: Message) -> None:
    if not msg.from_user:
        return
    uid = msg.from_user.id
    user = await get_user(uid)
    today = now_dt().date()
    raw_last = user.get("last_bonus_date")
    streak = int(user.get("streak_count") or 0)
    if raw_last:
        try:
            last = datetime.strptime(str(raw_last), "%Y-%m-%d").date()
        except ValueError:
            last = None
        if last == today:
            await msg.reply_text("🎁 Today's bonus has already been claimed.")
            return
    ...
    await update_user(
        uid,
        last_bonus_date=today.isoformat(),
        streak_count=streak,
        bonus_credits=int(user.get("bonus_credits") or 0) + 1,
    )
    await audit(uid, "daily_bonus", uid, {"streak": streak})
    await msg.reply_text(f"🎁 +1 bonus credit added.\n🔥 Streak: **{streak} day(s)**")


@bot.on_message(filters.command("mythumb") & filters.private)
async def mythumb_handler(_: Client, msg: Message) -> None:
    if not msg.from_user:
        return
    user = await get_user(msg.from_user.id)
    sent = False
    doc = get_valid_thumb(user.get("doc_thumb"))
    vid = get_valid_thumb(user.get("vid_thumb"))
    if doc:
        await msg.reply_photo(doc, caption="🖼 Document/Audio thumbnail")
        sent = True
    if vid:
        await msg.reply_photo(vid, caption="🎬 Video thumbnail")
        sent = True
    if not sent:
        await msg.reply_text("❌ No custom thumbnail is set.")


@bot.on_message(filters.command("clonehelp") & filters.private)
async def clonehelp_handler(_: Client, msg: Message) -> None:
    await msg.reply_text(
        "♻️ **FULL CLONE HELP**\n\n"
        "• `/clone <start_post_link>` — clones from that message to the latest accessible message into your bot chat.\n"
        "• `/clone <source> <destination> 500` — channel/chat transfer with a limit.\n"
        "• `/clone <source> <destination> all` — process all accessible history.\n\n"
        "Source admin is not required. Your connected account must legitimately be able to read the source. "
        "For a destination, Telegram must allow that account to post there."
    )
...
async def batch_handler(_: Client, msg: Message) -> None:
    ...
    if user.get("plan") not in {"Standard", "Premium", "Ultimate"} and not is_admin_id(msg.from_user.id):
        ...
        return
    if not await usage_available_or_notify(msg.from_user.id, msg):
        return
    links = extract_telegram_links(msg.text or "")
    if len(links) >= 2:
        a = parse_tg_link_ex(links[0])
        b = parse_tg_link_ex(links[1])
        if a and b and a[0] == b[0]:
            if a[1] != b[1]:
                await msg.reply_text("❌ Both range links must belong to the same Telegram topic/thread.")
                return
            start_id, end_id = sorted((a[2], b[2]))
            if end_id - start_id + 1 > MAX_BATCH_SIZE:
                await msg.reply_text(f"❌ Maximum batch size is {MAX_BATCH_SIZE} items.")
                return
            job_id = await enqueue_job(
                msg.from_user.id, "batch", a[0], start_id, end_id,
                payload={"topic_id": a[1]},
            )
...
async def transfer_handler(_: Client, msg: Message) -> None:
    ...
    if user.get("plan") != "Ultimate" and not is_admin_id(msg.from_user.id):
        await msg.reply_text("🔒 Full Clone is available on Ultimate plan.")
        return
    ...
    # /clone <post_link> = true start-link -> latest accessible message clone to this bot chat.
    if command == "/clone" and len(parts) == 2:
        parsed = parse_tg_link_ex(parts[1])
        if not parsed:
            await msg.reply_text(
                "Usage: `/clone <telegram_post_link>` or `/clone <source> <destination> [limit|all]`"
            )
            return
        chat_raw, topic_id, start_id = parsed
        job_id = await enqueue_job(
            msg.from_user.id, "clone_full", chat_raw, start_id, None,
            payload={"topic_id": topic_id},
        )
        await msg.reply_text(
            f"🚀 **Full Clone queued:** `{job_id}`\n"
            f"Start: `#{start_id}` → latest accessible message"
            + (f"\nTopic: `{topic_id}`" if topic_id else "")
        )
        return
    ...
    if len(parts) < 3:
        await msg.reply_text(
            "Usage:\n"
            "• `/clone <start_post_link>` — start → latest, sends to your bot chat\n"
            "• `/clone <source> <destination> [limit|all]`\n"
            "• `/transfer <source> <destination> [limit|all]`\n\n"
            "Source only needs legitimate read access. Destination needs Telegram-granted posting permission."
        )
        return
    ...
    if len(parts) >= 4:
        ...
        if token == "all":
            ...
            limit = max(1, min(int(token), 20000))
    job_id = await enqueue_job(
        msg.from_user.id, "transfer", src, destination=dest, payload={"limit": limit}
    )
    await msg.reply_text(
        f"✅ Channel clone queued: `{job_id}`\nLimit: **{'ALL' if limit == 0 else limit}**"
    )
...
@bot.on_message(filters.command(["ps", "authusers"]) & filters.private)
async def legacy_admin_info_handler(_: Client, msg: Message) -> None:
    if not msg.from_user or not await has_admin_role(msg.from_user.id, "manager", "payments", "support"):
        return
    ...
    if cmd == "ps":
        ...
        await msg.reply_text(
            f"👤 `{uid}`\nPlan: **{user.get('plan')}**\n"
            f"Expiry: **{human_dt(user.get('plan_expires_at'))}**\n"
            f"Used: **{user.get('daily_used', 0)}** / **{user.get('daily_limit', 0)}**\n"
            f"Bonus: **{user.get('bonus_credits', 0)}**"
        )
        return
    async with db_conn() as db:
        rows = await (await db.execute(
            "SELECT user_id,name,plan,plan_expires_at FROM users WHERE plan!='Free' ORDER BY user_id DESC LIMIT 50"
        )).fetchall()
    if not rows:
        await msg.reply_text("No paid/trial users found.")
        return
    body = ["👑 **ACTIVE/NON-FREE USERS**", ""]
    for r in rows:
        body.append(f"`{r['user_id']}` • {r['name'] or 'User'} • **{r['plan']}** • {human_dt(r['plan_expires_at'])}")
    await msg.reply_text("\n".join(body)[:4096])


@bot.on_message(filters.command("setupi") & filters.private)
async def setupi_handler(_: Client, msg: Message) -> None:
    if not msg.from_user or not await has_admin_role(msg.from_user.id, "manager", "payments"):
        return
    args = (msg.text or "").split(maxsplit=2)
    if len(args) < 3:
        await msg.reply_text("Usage: `/setupi <upi_id> <payee name>`")
        return
    await set_setting("upi_id", args[1].strip())
    await set_setting("upi_name", args[2].strip())
    await audit(msg.from_user.id, "payment_config_changed", details={"upi_id": args[1].strip()})
    await msg.reply_text("✅ UPI ID and payee name updated.")


@bot.on_message(filters.command("setprice") & filters.private)
async def setprice_handler(_: Client, msg: Message) -> None:
    if not msg.from_user or not await has_admin_role(msg.from_user.id, "manager", "payments"):
        return
    args = (msg.text or "").split()
    if len(args) < 4 or not args[3].isdigit():
        await msg.reply_text("Usage: `/setprice <Basic|Standard|Premium|Ultimate> <Weekly|Monthly|Yearly> <amount>`")
        return
    plan = args[1].capitalize()
    duration = args[2].capitalize()
    if plan not in PLAN_PRICING or duration not in DURATION_DAYS:
        await msg.reply_text("❌ Invalid plan or duration.")
        return
    amount = max(0, int(args[3]))
    await set_setting(price_setting_key(plan, duration), amount)
    await audit(msg.from_user.id, "price_changed", plan, {"duration": duration, "amount": amount})
    await msg.reply_text(f"✅ {plan} {duration} price updated to ₹{amount}.")
    ...
    try:
        ...
        if data == "home":
            ...
            if plan not in PLAN_PRICING:
                ...
                return
            p = {d: await get_plan_price(plan, d) for d in DURATION_DAYS}
            markup = InlineKeyboardMarkup([
            ...
            caption = (
                ...
                f"UPI ID: `{upi_id}`\nPayee: **{upi_name}**\n"
                f"Admin: **@{ADMIN_USERNAME_ENV}**\n\n"
                "📸 **Step 1:** Complete payment and send the payment screenshot here as a PHOTO.\n"
                "🔢 **Step 2:** After the screenshot is accepted, send the exact **12-digit UPI UTR/RRN**.\n"
                "🛡️ **Step 3:** Plan activates only after the admin verifies that the money was actually received.\n\n"
                "A UTR number or screenshot alone is not treated as proof of payment."
            )
            ...
            if not rows:
                ...
                for r in rows:
                    markup = InlineKeyboardMarkup([[
                        InlineKeyboardButton("🔎 Verify", callback_data=f"pay:ok:{r['invoice_id']}"),
            ...
            if not payment or payment.get("status") != "pending":
                await safe_answer(q, "Payment is not pending.", True)
                return
            if not payment.get("screenshot_file_id"):
                await safe_answer(q, "Screenshot proof is missing.", True)
                return
            markup = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Money received — ACTIVATE", callback_data=f"pay:confirm:{invoice_id}")],
                [InlineKeyboardButton("❌ Reject", callback_data=f"pay:no:{invoice_id}")],
            ])
            await q.message.reply_text(
                f"🔎 **FINAL PAYMENT CHECK**\n\nInvoice: `{invoice_id}`\n"
                f"UTR: `{payment.get('utr')}`\nAmount: **₹{payment.get('amount')}**\n\n"
                "Open your bank/UPI app and confirm the money was actually credited.\n"
                "Only then tap **Money received — ACTIVATE**.",
                reply_markup=markup,
            )
...
async def payment_screenshot_handler(_: Client, msg: Message) -> None:
    if not msg.from_user or not msg.photo:
        return
    user_id = msg.from_user.id
    state = LOGIN_STATES.get(user_id)
    if not state or state.get("step") != "PAYMENT_SCREENSHOT":
        return
    invoice_id = str(state.get("invoice_id") or "")
    payment = await get_payment(invoice_id)
    if not payment or int(payment.get("user_id") or 0) != user_id or payment.get("status") != "awaiting_utr":
        LOGIN_STATES.pop(user_id, None)
        await msg.reply_text("❌ This payment invoice is no longer waiting for a screenshot.")
        return
    file_id = msg.photo.file_id
    async with DB_LOCK:
        async with db_conn() as db:
            await db.execute(
                "UPDATE payments SET screenshot_file_id=?, screenshot_message_id=? WHERE invoice_id=? AND user_id=?",
                (file_id, msg.id, invoice_id, user_id),
            )
            await db.commit()
    LOGIN_STATES[user_id] = {
        "step": "PAYMENT_UTR",
    ...
    await msg.reply_text(
        ...
        "Now send the exact **12-digit UPI UTR/RRN**.\n"
        "Example: `123456789012`\n\n"
        "🛡️ The subscription remains pending until the admin verifies the payment in the bank/UPI account."
    )
...
async def text_handler(_: Client, msg: Message) -> None:
    ...
    if state:
        ...
        if step == "PHONE":
            ...
            try:
                ...
                await msg.reply_text(
                    ...
                    "Send the Telegram OTP digits here. Spaces are allowed.\n"
                    "Example: `1 4 2 0 4`\n\n"
                    "🔐 If 2-Step Verification is enabled, the bot will ask for your 2FA password next.\n"
                    "⚠️ Never share this OTP with any person. Use `/cancel` to stop login."
                )
            except Exception as exc:
                try:
        ...
        if step == "BATCH_START":
            ...
            await msg.reply_text(f"✅ Start saved: `#{parsed[2]}`\nNow send item count (1–{MAX_BATCH_SIZE}).")
            return
        ...
        if step == "BATCH_COUNT":
            ...
            LOGIN_STATES.pop(user_id, None)
            if not await usage_available_or_notify(user_id, msg):
                return
            try:
                job_id = await enqueue_job(
                    user_id, "batch", chat_raw, start_id, end_id,
                    payload={"topic_id": topic_id},
                )
        ...
        if step == "PAYMENT_SCREENSHOT":
            await msg.reply_text(
                "📸 Please send the **payment screenshot as a photo** first.\n"
                "Do not send the UTR until the screenshot is accepted. Use `/cancel` to stop."
            )
            return
        ...
        if step == "PAYMENT_UTR":
            ...
            payment = await get_payment(invoice_id)
            await msg.reply_text(
                f"✅ `{invoice_id}` submitted.\n"
                "🛡️ Payment is **pending manual admin verification**.\n"
                "Your plan will NOT activate just because a 12-digit number was submitted."
            )
            if payment:
                markup = InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔎 Verify Payment", callback_data=f"pay:ok:{invoice_id}"),
                    InlineKeyboardButton("❌ Reject", callback_data=f"pay:no:{invoice_id}"),
                ]])
                photo_caption = (
                    f"📸 **PAYMENT SCREENSHOT**\n\nInvoice: `{invoice_id}`\nUser: `{user_id}`\n"
                    f"Plan: **{payment['plan']}** • **{payment['duration']}**\nAmount: **₹{payment['amount']}**"
                )
                utr_alert = (
                    f"💳 **PAYMENT UTR — MANUAL VERIFICATION REQUIRED**\n\n"
                    f"Invoice: `{invoice_id}`\nUser: `{user_id}`\n"
                    f"UTR/RRN: `{payment['utr']}`\nAmount: **₹{payment['amount']}**\n\n"
                    "⚠️ Verify the actual credit in your bank/UPI account before approving. "
                    "A screenshot or UTR can be forged."
                )
                recipients = await payment_admin_recipient_ids()
                delivered = 0
                for a in recipients:
                    try:
                        if payment.get("screenshot_file_id"):
                            await bot.send_photo(a, payment["screenshot_file_id"], caption=photo_caption)
                        await bot.send_message(a, utr_alert, reply_markup=markup)
                        delivered += 1
                    except Exception as exc:
                        log.warning("Payment alert delivery failed for admin %s: %s", a, exc)
                if delivered == 0:
                    log.error(
                        "No payment admin could be notified for invoice %s. Configure OWNER_ID/ADMIN_ID or a reachable ADMIN_USERNAME.",
                        invoice_id,
                    )
            return
    ...
    if "t.me/" in text:
        ...
        if not await usage_available_or_notify(user_id, msg):
            return
        # Optional compact range syntax for non-topic links: https://t.me/channel/100-120
        ...
        try:
            ...
            parsed = parse_tg_link_ex(text)
            if parsed:
                chat_raw, topic_id, msg_id = parsed
                job_id = await enqueue_job(
                    user_id, "single", chat_raw, msg_id, msg_id,
                    payload={"topic_id": topic_id},
                )
...
async def subscription_reminder_job() -> None:
    ...
    for row in rows:
        ...
        # Expiration is enforced even when notification preferences are disabled.
        ...
        try:
            ...
            await bot.send_message(
                int(row["user_id"]),
                f"⏰ Your **{row['plan']}** plan expires in about {days} day(s). Use `/plans` to renew.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("💎 Renew / Buy", callback_data="home_plans")
                ]]),
            )
        except Exception:
            pass
...
async def register_commands() -> None:
    commands = [
        ...
        BotCommand("login", "Connect account: Session or Phone + OTP"),
        BotCommand("logout", "Disconnect account"),
        BotCommand("help", "Complete user guide"),
        BotCommand("id", "Show your Telegram user ID"),
        BotCommand("batch", "Range batch download"),
        BotCommand("clone", "Full clone from start link / channel clone"),
        BotCommand("clonehelp", "Clone usage examples"),
        BotCommand("transfer", "Channel/chat transfer (Ultimate)"),
        BotCommand("jobs", "Recent jobs and progress"),
        BotCommand("pause", "Pause a running/queued job"),
        BotCommand("resume", "Resume a paused job"),
        ...
        BotCommand("referral", "Referral link and credits"),
        BotCommand("bonus", "Daily bonus credit"),
        BotCommand("mythumb", "View custom thumbnails"),
...
async def start_services() -> None:
    ...
    SCHEDULER.add_job(subscription_reminder_job, "interval", minutes=5, id="subscription_reminders", replace_existing=True)
    SCHEDULER.add_job(cleanup_temp_job, "interval", hours=6, id="temp_cleanup", replace_existing=True)
    SCHEDULER.start()

    await bot.start()
    await resolve_configured_admin_username()
    me = await bot.get_me()
...
def main() -> None:
    if not API_ID or not API_HASH or not BOT_TOKEN:
        print("FATAL: API_ID, API_HASH and BOT_TOKEN must be configured.")
        raise SystemExit(2)
    if not OWNER_ID and not ADMIN_USERNAME_ENV:
        print("FATAL: Configure OWNER_ID/ADMIN_ID or ADMIN_USERNAME for admin/payment verification.")
        raise SystemExit(2)
