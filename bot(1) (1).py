#!/usr/bin/env python3
"""
SavePro V3 Main Engine Advanced
======================
...
APP_NAME = os.getenv("APP_NAME", "SavePro V3 Main Engine Advanced").strip()
...
async def execute_transfer_job(job: dict[str, Any]) -> None:
    """Clone accessible source history using the connected user session. Source needs read access; destination needs Telegram-granted posting permission."""
    user_id = int(job["user_id"])
    user = await get_user(user_id)
    ...
    job_id = str(job["job_id"])
    status_msg = await bot.send_message(user_id, f"🔄 **{job_id}** validating source access and destination posting permission…")
    u_client: Optional[Client] = None
    job_dir = TEMP_DIR / job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    success = failed = 0
    try:
        ...
        # Source only needs to be accessible to the connected user (public, or private
        # where that account is already a legitimate member). Destination needs write/admin rights.
        try:
            await u_client.get_chat(src)
        except Exception as exc:
            raise PermissionError("Source chat is not accessible to your connected Telegram account.") from exc
        # Do not require an "admin" title here. Telegram itself decides whether
        # the connected account may post to the destination. This supports groups
        # where ordinary members have send permission, while channels that require
        # elevated posting rights will naturally reject the send operation.
        try:
            await u_client.get_chat(dest)
        except Exception as exc:
            raise PermissionError("Destination chat is not accessible to your connected Telegram account.") from exc
...
async def transfer_handler(_: Client, msg: Message) -> None:
    ...
    if user.get("plan") != "Ultimate" and not is_admin_id(msg.from_user.id):
        await msg.reply_text("🔒 Full channel clone is available on Ultimate plan.")
        return
    parts = msg.text.split()

    # Backward-compatible direct clone: /clone <post_link>
    if len(parts) == 2 and parts[0].split("@")[0].lower() == "/clone":
        parsed = parse_tg_link(parts[1])
        if not parsed:
            await msg.reply_text("Usage: `/clone <telegram_post_link>` or `/clone <source> <destination> [limit]`")
            return
        chat_raw, start_id = parsed
        clone_count = min(75, MAX_BATCH_SIZE)
        end_id = start_id + clone_count - 1
        job_id = await enqueue_job(msg.from_user.id, "batch", chat_raw, start_id, end_id)
        await msg.reply_text(
            f"🚀 Direct clone queued: `{job_id}`\n"
            f"Starting at #{start_id}, up to {clone_count} accessible posts will be processed to this bot chat."
        )
        return

    if len(parts) < 3:
        await msg.reply_text(
            "Usage: `/clone <telegram_post_link>` or `/transfer <source> <destination> [limit]`\n\n"
            "Source must be accessible to your connected account; destination requires Telegram-granted posting permission."
        )
        return
    ...
    await msg.reply_text(f"✅ Full channel clone queued: `{job_id}`")
